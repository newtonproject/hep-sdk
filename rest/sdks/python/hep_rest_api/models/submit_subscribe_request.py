...
class SubmitSubscribeRequest(object):
    ...
    def tx(self, tx):
        ...
        if tx is not None and len(tx) > 256:
            raise ValueError("Invalid value for `tx`, length must be less than or equal to `256`")  # noqa: E501
        if tx is not None and len(tx) < 1:
            raise ValueError("Invalid value for `tx`, length must be greater than or equal to `1`")  # noqa: E501
