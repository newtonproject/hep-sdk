...
class ReceiptField(object):
    ...
    def proof_status(self, proof_status):
        ...
        if proof_status is not None and len(proof_status) > 64:
            raise ValueError("Invalid value for `proof_status`, length must be less than or equal to `64`")  # noqa: E501
        if proof_status is not None and len(proof_status) < 1:
            raise ValueError("Invalid value for `proof_status`, length must be greater than or equal to `1`")  # noqa: E501
